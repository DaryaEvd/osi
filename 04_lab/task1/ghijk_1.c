...
int main(int argc, char **argv, char **envp) {
  /*
      - Как работает envp под капотом? 
      - Как убедиться в том, где хранятся переменные окружения?
          Типа кроме dump-a как посмотреть что они расположены на дне стека?
      - Как конфигурируются переменные окружения? 
      - Как хранятся переменные окружения? Где?
  */
